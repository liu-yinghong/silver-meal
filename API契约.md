...
| 1a | GET | `/api/meals/{meal_id}` | 按 ID 查询单份餐食（再来一份） | 老人端 |
| 2 | POST | `/api/meals/today` | 今日推荐：评分最高 + 大模型天气 + 历史下单 | 老人端 |
| 3 | POST | `/api/meals/recommend/stream` | 大模型工作流 SSE 流式（4步实时进度） | 老人端 |
| 3a | POST | `/api/meals/recommend/local` | 本地传统检索（不调用大模型） | 老人端 |
...
| 18 | GET | `/api/weather` | 获取实时天气（智能体工具） | 老人端 |
| 19 | POST | `/api/asr` | 语音转文字（大模型 ASR） | 老人端 |
| 20 | GET | `/health` | 健康检查 | 两端 |
...
> **预算硬过滤（2026-08-02 更新）**：推荐链路对预算做**硬过滤**——家属规则 `max_price` 与用户输入中解析出的价格上限（如"30元以内"→`price_max`）都会**直接剔除超预算餐品**，绝不推荐超出用户预算的餐食。若所有餐品均超预算，返回 422 `NO_ELIGIBLE_MEAL`。
>
> 例：输入"咖喱鸡肉饭，20元以内"，28 元的咖喱鸡肉饭会被排除，只推荐 ≤20 元的餐品。
...
响应（200）：单份餐食对象（字段同接口 1 的 `meals` 元素）：
```json
{
  "id": "meal_001",
  "image_id": "meal_001_ui",
  "name": "清蒸鲈鱼套餐",
  "description": "清蒸鲈鱼配时蔬和米饭，低油低盐，适合清淡饮食",
  "price": 28.0,
  "image_url": "/elder/images/meal_001_ui.png",
  "dietary_tags": ["low_oil", "low_salt"],
  "calories": 450,
  "eta_minutes": 35
}
```
...
> 用途：老人端"再来一份"点击后直接查询上次下单的餐食并优先展示。
...
- `lat`/`lon`：用户位置（用于天气推荐），可选，不传则用后端默认城市

响应（200）：结构同接口 1（`meals`/`query_summary`/`reasons`/`ai_mode`）。三份餐食分别为：
1. **评分最高**：现有推荐引擎综合评分第一（含大模型意图解析 + 家属规则过滤）
2. **天气推荐**：大模型按用户位置实时天气挑选，失败降级为本地规则映射
3. **历史下单最多**：按订单历史统计下单次数最多的餐食

错误：422 `NO_ELIGIBLE_MEAL`（无符合家庭规则的餐品）

---

### 1.6 大模型推荐工作流（SSE 流式）

**POST** `/api/meals/recommend/stream`
...
- `mode`：`input`（文字/语音输入）或 `today`（今日推荐）
- 响应为 `text/event-stream`，逐条推送工作流进度事件（每步先 `running` 后 `done`）：

```json
// data: {"step":1,"status":"running","detail":"正在理解您说的口味、预算和偏好..."}
// data: {"step":1,"status":"done","detail":"已理解需求：偏好低油、低盐"}
// data: {"step":2,"status":"done","detail":"家属规则：单餐最高 35 元；允许低油、低盐"}
// data: {"step":3,"status":"done","detail":"已分析 50 份餐品，3 份符合要求"}
// data: {"step":4,"status":"done","detail":"已生成推荐方案","result":{"meals":[...],"query_summary":"...","reasons":[...],"ai_mode":"remote"}}
```

四步依次为：**1 理解需求**（大模型意图解析）→ **2 查看家庭设置**（读取家属规则）→ **3 分析餐品**（评分筛选）→ **4 生成方案**（输出推荐结果）。前端按事件实时打勾展示大模型推理进度。

错误步骤事件：`{"step":3,"status":"error","detail":"..."}` 或 `{"step":4,"status":"error","detail":"..."}`

---

### 1.7 本地传统检索（不调用大模型）

**POST** `/api/meals/recommend/local`

请求：同接口 1（`text_input` / `family_id`）。

响应（200）：结构同接口 1，但 `ai_mode` 恒为 `local`。

用途：当大模型响应较慢或不可用时，前端"用本地推荐继续"降级按钮调用此接口。逻辑：
1. 按用户输入做**本地关键词解析**（口味/预算/食材关键词，不调用大模型）
2. 应用**家属规则**硬过滤（预算上限、禁止食材、允许偏好）
3. 应用**用户输入的价格上限**硬过滤（如"30元以内"，超预算餐品直接剔除）
4. 综合评分（关键词匹配 + 标签匹配 + 规则符合度）
5. 按评分降序取前三返回
...
> `meal_id`：餐食 ID（用于"再来一份"直接查询上次下单的餐食，配合 `GET /api/meals/{meal_id}` 使用）
...
> `sender`（可选）：发送家属称呼，如"女儿"；不传则老人端显示通用"家人"。
...
```
> `sender`：发送家属称呼；历史消息可能为空，老人端需回退显示"家人"。
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `lat` | number | 否 | 纬度，不传则用后端默认城市（上海） |
| `lon` | number | 否 | 经度，不传则用后端默认城市 |
...
字段说明：
- `date`：实时日期（月+日+星期）
- `temp`：实时气温（°C），获取失败时为 `null`
- `condition` / `icon`：天气描述与图标
- `source`：`open-meteo`（真实数据）或 `fallback`（降级）
- `location`：展示的城市名

> 数据来源：Open-Meteo 免费 API（无需 Key）。前端通过浏览器定位获取实地坐标后调用，定位失败则用默认城市。任何网络失败自动降级，不影响主流程。
...
请求：上传音频文件，字段名 `file`，格式要求 **16kHz 单声道 PCM WAV**。
...
错误：
- 422 `ASR_FAILED`：识别失败或未识别到语音（前端应提示用户直接输入文字）

> 数据来源：阿里云百炼 DashScope Paraformer（`paraformer-realtime-v2`），API Key 复用 `OPENAI_API_KEY`。前端按下录音 → 录制 WAV → 上传后端识别 → 返回文字进入推荐流程。
