...
const API = (function () {
   ...
   * @param {number|null} lat - 纬度（可选，不传则用后端默认城市）
   * @param {number|null} lon - 经度（可选）
   * @returns {Promise<{data: object|null, error: object|null}>}
   *
   * 成功响应示例：
   * { date: "8月1日 周六", temp: 32, condition: "多云", icon: "⛅", source: "open-meteo", location: "上海" }
   */
  async function getWeather(lat, lon) {
    const qs = [];
    if (lat != null && lon != null) {
      qs.push('lat=' + encodeURIComponent(lat), 'lon=' + encodeURIComponent(lon));
    }
    return request('/api/weather' + (qs.length ? '?' + qs.join('&') : ''));
  }

  /**
   * 语音识别 — 大模型 ASR（DashScope Paraformer）
   * POST /api/asr （multipart/form-data，字段名 file，WAV 格式）
   * @param {Blob} wavBlob - 16kHz 单声道 PCM WAV 音频
   * @returns {Promise<{data: object|null, error: object|null}>}
   *
   * 成功响应示例：{ text: "想吃清淡的" }
   */
  async function transcribeAudio(wavBlob) {
    ...
    return request('/api/asr', { method: 'POST', body: form });
  }
   ...
   * 成功响应示例：
   * {
   *   meals: [{ id, image_id, name, description, price, image_url, dietary_tags, calories, eta_minutes }],
   *   query_summary: "您说：...",
   *   reasons: ["..."],
  ...
  /**
   * 本地传统检索推荐（不调用大模型）— AI 响应慢时的降级
   * POST /api/meals/recommend/local
   * @param {string} textInput - 用户输入的需求文字
   * @returns {Promise<{data: object|null, error: object|null}>}
   */
  async function recommendLocal(textInput) {
    return request('/api/meals/recommend/local', {
      method: 'POST',
      body: {
        text_input: textInput,
        family_id: DEMO_FAMILY_ID,
  ...
  /**
   * 今日推荐 — 大模型综合推荐三份餐食（评分最高 + 天气 + 历史下单）
   * POST /api/meals/today
   * @param {number|null} lat - 纬度（用于天气推荐）
   * @param {number|null} lon - 经度
   * @returns {Promise<{data: object|null, error: object|null}>}
   */
  async function todayRecommend(lat, lon) {
    return request('/api/meals/today', {
      method: 'POST',
      body: {
        family_id: DEMO_FAMILY_ID,
  ...
  /**
   * 按 ID 查询单份餐食（"再来一份"直接展示上次下单的餐食）
   * GET /api/meals/{meal_id}
   * @param {string} mealId
   * @returns {Promise<{data: object|null, error: object|null}>}
   */
  async function getMealById(mealId) {
    return request('/api/meals/' + encodeURIComponent(mealId));
  }
