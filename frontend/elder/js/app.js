...
// ================================================================
(function(){
var DOM={}, isRecording=false, recentOrder=null, voiceMaxTimer=null;
...
function startCapture(){
  ...
  navigator.mediaDevices.getUserMedia({audio:true}).then(function(stream){
    ...
    isRecording=false; resetVoiceUI();
    if(err&&err.name==='NotAllowedError') showVoiceError('麦克风权限被拒绝。\n请点击浏览器地址栏的麦克风图标，允许访问后重试。');
...
function fallbackStartCapture(stream){
  ...
  try{
    ...
    releaseCapture(); resetVoiceUI(); showVoiceError('录音初始化失败，请直接输入文字');
  }
}
...
PageManager.pageHooks.home = {
  ...
  onLeave: function(){
    ...
    if(isRecording){
      if(voiceMaxTimer){ clearTimeout(voiceMaxTimer); voiceMaxTimer=null; }
      releaseCapture(); pcmSamples=[]; resetVoiceUI();
    }
  }
};
